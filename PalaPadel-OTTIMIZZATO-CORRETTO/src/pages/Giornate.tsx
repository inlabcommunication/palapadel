...
export function GiornatePage() {
  ...
  const [activeTab, setActiveTab] = useState<"standings" | "bracket" | "calendar">(
    searchParams.get("view") === "calendar" ? "calendar" : "standings"
  );
  ...
  return (
    <div className="p-4 pb-6">
      ...
      <div className="mb-4 mt-3 grid grid-cols-2 rounded-lg bg-[#0A0B08] p-1" role="tablist" aria-label="Area campionato">
        ...
        <button role="tab" aria-selected={activeTab === "bracket"} onClick={() => setActiveTab("bracket")}
          className={`rounded-md px-3 py-2.5 text-sm font-bold ${activeTab === "bracket" ? "bg-[#BBFF5E] text-[#081208]" : "text-[rgba(251,243,222,0.72)]"}`}>
          Tabellone
        </button>
      </div>
      ...
      <button
        ...
        className={`mb-4 flex w-full items-center justify-center gap-2 rounded-lg border px-3 py-2.5 text-sm font-bold ${
          activeTab === "calendar" ? "border-[#BBFF5E] bg-[rgba(187,255,94,0.08)] text-[#BBFF5E]" : "border-[rgba(251,243,222,0.16)] text-[#FBF3DE]"
        }`}
      >
        <CalendarDays size={16} /> Gestione calendario
      </button>
