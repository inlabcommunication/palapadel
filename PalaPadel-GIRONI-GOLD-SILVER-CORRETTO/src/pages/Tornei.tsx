        ...
        <table className={`w-full text-sm ${isOperator ? "min-w-[620px]" : "table-fixed"}`}>
          <thead className="text-[10px] uppercase text-[rgba(251,243,222,0.48)]">
            <tr>
              ...
              {isOperator && <th>{tournament.bracketMode === "gold_silver" ? "Tabellone" : "Passata"}</th>}
              {isOperator && <th />}
            </tr>
          </thead>
          <tbody>
            {sorted.map((entry) => <GroupTeamRow key={entry.id} tournamentId={tournament.id} bracketMode={tournament.bracketMode} entry={entry} name={teamName(entry)} canEdit={isOperator} showOperational={isOperator} canRemove={isOperator} notify={notify} />)}
          </tbody>
        </table>
      </div>
      {isOperator && (
...
// Compatibilità temporanea con il rendering precedente; il nuovo componente è definito subito sotto.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function LegacyGroupTeamRow({ tournamentId, bracketMode, entry, name, canEdit, showOperational, canRemove, notify }: {
  tournamentId: string; bracketMode: Tournament["bracketMode"]; entry: TournamentGroupTeam; name: string; canEdit: boolean; showOperational: boolean; canRemove: boolean; notify: (message: string) => void;
}) {
  const [draft, setDraft] = useState(entry);
  const save = async () => {
    try {
      await updateTournamentGroupTeam(tournamentId, entry.id, { played: draft.played, won: draft.won, lost: draft.lost, points: draft.points, order: draft.order, qualified: draft.qualified, qualificationBracket: draft.qualificationBracket ?? null });
  ...
  return (
    <tr className={`border-t border-[rgba(251,243,222,0.08)] ${
      !showOperational && entry.qualificationBracket === "gold"
        ? "bg-[rgba(212,175,55,0.16)]"
        : !showOperational && entry.qualificationBracket === "silver"
          ? "bg-[rgba(192,192,192,0.14)]"
          : entry.qualified ? "bg-[rgba(187,255,94,0.06)]" : ""
    }`}>
...
function GroupTeamRow({ tournamentId, bracketMode, entry, name, canEdit, showOperational, canRemove, notify }: {
  tournamentId: string;
  bracketMode: Tournament["bracketMode"];
  entry: TournamentGroupTeam;
  name: string;
  canEdit: boolean;
  ...
  const save = async (next = draft) => {
    try {
      await updateTournamentGroupTeam(tournamentId, entry.id, {
        played: next.played,
        won: next.won,
        lost: next.lost,
        points: next.points,
        ...
        qualificationBracket: next.qualificationBracket ?? null,
      });
      notify("Girone aggiornato.");
    } catch (error) {
      notify(error instanceof Error ? error.message : "Errore.");
    }
  };
  const number = (field: "played" | "points") => canEdit ? (
    <input
      ...
      onBlur={() => void save()}
      className="w-14 rounded border border-[rgba(251,243,222,0.12)] bg-[#123008] px-2 py-1 text-center"
    />
  ) : draft[field];
  const publicTone = entry.qualificationBracket === "gold"
    ? "bg-[rgba(212,175,55,0.16)]"
    : entry.qualificationBracket === "silver"
      ? "bg-[rgba(192,192,192,0.14)]"
      : entry.qualified ? "bg-[rgba(187,255,94,0.06)]" : "";

  return (
    <tr className={`border-t border-[rgba(251,243,222,0.08)] ${showOperational ? "" : publicTone}`}>
      <td className="break-words px-3 py-3 font-bold">
        {name}
        {!showOperational && entry.qualificationBracket && (
          <span className={`ml-2 text-[10px] font-extrabold uppercase ${entry.qualificationBracket === "gold" ? "text-[#D4AF37]" : "text-[#C0C0C0]"}`}>
            {entry.qualificationBracket}
          </span>
        )}
      </td>
      <td className="text-center">{number("played")}</td>
      ...
      {showOperational && (
        <td className="px-2 text-center">
          {bracketMode === "gold_silver" ? (
            <select
              aria-label={`Tabellone di qualificazione ${name}`}
              value={draft.qualificationBracket ?? ""}
              onChange={async (event) => {
                const destination = event.target.value as "" | "gold" | "silver";
                const next = { ...draft, qualified: Boolean(destination), qualificationBracket: destination || undefined };
                setDraft(next);
                await save(next);
              }}
              className="rounded border border-[rgba(251,243,222,0.14)] bg-[#123008] px-2 py-1.5 text-xs font-bold"
            >
              <option value="">Non passata</option>
              <option value="gold">Gold</option>
              <option value="silver">Silver</option>
            </select>
          ) : (
            <input
              aria-label={`Qualificata ${name}`}
              type="checkbox"
              checked={draft.qualified}
              onChange={async (event) => {
                const next = { ...draft, qualified: event.target.checked, qualificationBracket: undefined };
                setDraft(next);
                await save(next);
              }}
            />
      ...
      {canRemove && (
        <td className="px-3 text-right">
          <button title="Rimuovi coppia" onClick={() => removeTournamentGroupTeam(tournamentId, entry.id)} className="text-[#FF6B6B]"><X size={15} /></button>
        </td>
      )}
    </tr>
  );
