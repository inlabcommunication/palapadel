...
export type ContextNotificationKind = "standings" | "calendar" | "news";

export function getContextNotificationStatus() {
  return postToBackend<{
    ok: true;
    since: string;
    changesCount: number;
    detected: Record<ContextNotificationKind, boolean>;
  }>("/api/notifications/context-status", {});
}

export function sendContextNotifications(kinds: ContextNotificationKind[]) {
  return postToBackend<{
    ok: true;
    sentAt: string;
    results: Array<{ kind: ContextNotificationKind; status: string; successCount: number; failureCount: number }>;
  }>("/api/notifications/context-send", { kinds });
}
