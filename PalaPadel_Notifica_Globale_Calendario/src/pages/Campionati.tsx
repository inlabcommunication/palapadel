...
export function CampionatiPage() {
  ...
  const [contentTab, setContentTab] = useState<"standings" | "calendar" | "bracket">(
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                {isAdmin && activeType?.hasTeams && (
                  <button
                    onClick={() => navigate(`/gestione/edizione/${edition.id}?view=calendar`)}
                    className="flex items-center gap-1 text-xs font-semibold text-[#FBF3DE]"
                  >
                    <Calendar size={13} /> Gestione calendario
                  </button>
                )}
                {isAdmin && (
      ...
      {edition && activeType?.hasTeams && (
        <>
          <div className="mb-4 grid grid-cols-2 rounded-lg bg-[#0A0B08] p-1" role="tablist" aria-label="Contenuti del campionato">
            ...
            <button role="tab" aria-selected={contentTab === (isAdmin ? "bracket" : "calendar")} onClick={() => setContentTab(isAdmin ? "bracket" : "calendar")}
              className={`rounded-md px-3 py-2.5 text-sm font-bold ${contentTab === (isAdmin ? "bracket" : "calendar") ? "bg-[#BBFF5E] text-[#081208]" : "text-[rgba(251,243,222,0.72)]"}`}>
              {isAdmin ? "Tabellone" : "Calendario"}
            </button>
          </div>
          {contentTab === "standings" ? (
            <>
              {!isAdmin && edition.bracketEnabled && (
                <button onClick={() => setContentTab("bracket")} className="mb-4 flex w-full items-center justify-center gap-2 rounded-lg border border-[#BBFF5E] px-4 py-3 text-sm font-bold text-[#BBFF5E]">
                  <Trophy size={16} /> Guarda tabellone
                </button>
              )}
            ...
            <div key={r.id} className="flex items-center px-3.5 py-2.5 text-[13px] border-b border-[rgba(251,243,222,0.08)] last:border-b-0">
              ...
              {isAdmin && (
                <div className="flex w-28 items-center justify-end gap-1">
                  <button
                    type="button"
                    aria-label={`Sposta ${r.team?.name ?? "squadra"} in alto tra le squadre a pari punti`}
                    disabled={reorderingTie || i === 0 || rows[i - 1]?.points !== r.points}
                    onClick={() => moveTiedTeam(r.id, -1)}
                    className="rounded p-1 text-[#BBFF5E] disabled:opacity-20"
                  >
                    <ChevronUp size={14} />
                  </button>
                  <button
                    type="button"
                    aria-label={`Sposta ${r.team?.name ?? "squadra"} in basso tra le squadre a pari punti`}
                    disabled={reorderingTie || i === rows.length - 1 || rows[i + 1]?.points !== r.points}
                    onClick={() => moveTiedTeam(r.id, 1)}
                    className="rounded p-1 text-[#BBFF5E] disabled:opacity-20"
                  >
                    <ChevronDown size={14} />
                  </button>
                  <button onClick={() => setEditingId(r.id)} className="text-[#BBFF5E] text-xs font-semibold">
                    Modifica
                  </button>
                </div>
              )}
  ...
  return (
    <div className="mt-4">
      ...
      <div className="relative mb-3">
        <label htmlFor={`public-matchday-${edition.id}`} className="mb-1.5 block text-[11px] font-bold uppercase text-[rgba(251,243,222,0.50)]">
          Giornata visualizzata
        </label>
        <select
          id={`public-matchday-${edition.id}`}
          value={activeMatchdayId ?? ""}
          onChange={(event) => setSelectedMatchdayId(event.target.value)}
          className="w-full appearance-none rounded-lg border border-[#BBFF5E] bg-[#BBFF5E] px-3 py-3 pr-10 text-sm font-bold text-[#081208]"
        >
          {sortedMatchdays.map((matchday) => (
            <option key={matchday.id} value={matchday.id}>{matchday.number}ª giornata</option>
          ))}
        </select>
        <ChevronDown className="pointer-events-none absolute bottom-3.5 right-3 text-[#081208]" size={16} />
      </div>
...
function EditionTeamEditRow({
  ...
  const save = async () => {
    ...
    try {
      ...
      await updateStandingsEntry({
        editionId,
        editionTeamId: editionTeam.id,
        baselinePoints: Math.max(0, (Number(points) || 0) - matchPoints),
        baselinePlayed: Math.max(0, (Number(played) || 0) - matchPlayed),
        manualPointsAdjustment: Math.min(0, (Number(points) || 0) - matchPoints),
        manualPlayedAdjustment: Math.min(0, (Number(played) || 0) - matchPlayed),
        order: editionTeam.order,
        reason: auditReason,
      });
  ...
  const remove = async () => {
    ...
    try {
      await removeStandingsEntry({ editionId, editionTeamId: editionTeam.id, reason: "Rimozione manuale dalla classifica" });
