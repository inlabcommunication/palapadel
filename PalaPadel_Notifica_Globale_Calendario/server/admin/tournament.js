...
const schema = z.discriminatedUnion("operation", [
  ...
  z.object({ operation: z.literal("updateGroupTeam"), tournamentId: documentId, entryId: documentId, played: z.number().int().min(0).max(999), won: z.number().int().min(0).max(999), lost: z.number().int().min(0).max(999), points: z.number().int().min(-999).max(9999), order: z.number().int().min(0), qualified: z.boolean(), qualificationBracket: z.enum(["gold", "silver"]).nullable().optional() }).strict(),
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (input.operation === "createTournament") {
        ...
        if (input.operation === "removeGroupTeam") {
          ...
          after = { ...before, played: input.played, won: input.won, lost: input.lost, points: input.points, order: input.order, qualified: input.qualified, qualificationBracket: input.qualificationBracket || null };
          transaction.update(ref, {
            ...
            qualificationBracket: input.qualificationBracket || admin.firestore.FieldValue.delete(),
          });
        }
