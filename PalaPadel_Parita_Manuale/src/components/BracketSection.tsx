...
const isSemifinal = (name: string) => name.trim().toLowerCase().startsWith("semi");
const isFinal = (name: string) => name.trim().toLowerCase() === "finale";
const FINAL_STAGES_VALUE = "__semifinale_finale__";
...
export function BracketSection({
  ...
  const finalStageRounds = sortedRounds.filter((round) => isSemifinal(round.name) || isFinal(round.name));
  const automaticRound = sortedRounds.find((round) => round.id === automaticRoundId);
  const showFinalStages = !isAdmin && (
    selectedRoundId === FINAL_STAGES_VALUE ||
    (selectedRoundId === null && automaticRound && (isSemifinal(automaticRound.name) || isFinal(automaticRound.name)))
  );
  ...
  if (isFrozen && !showLive) {
    ...
    const frozenSemifinalIndex = frozenRounds.findIndex((round) => isSemifinal(round.name));
    const frozenFinalIndex = frozenRounds.findIndex((round) => isFinal(round.name));
    const defaultFrozenIndex = frozenSemifinalIndex >= 0 && frozenFinalIndex >= 0
      ? frozenSemifinalIndex
      : Math.max(0, frozenRounds.length - 1);
    const frozenRoundIndex = selectedFrozenRoundIndex ?? defaultFrozenIndex;
    const frozenRound = frozenRounds[frozenRoundIndex];
    const showFrozenFinalStages = frozenRound && (isSemifinal(frozenRound.name) || isFinal(frozenRound.name)) && frozenSemifinalIndex >= 0 && frozenFinalIndex >= 0;
    const frozenRoundsToRender = showFrozenFinalStages
      ? [frozenRounds[frozenSemifinalIndex], frozenRounds[frozenFinalIndex]]
      : frozenRound ? [frozenRound] : [];
    const frozenFinal = frozenRounds[frozenFinalIndex];
    const frozenChampion = frozenFinal?.matches.find((match) => match.winnerSide);
    return (
      <div className="mt-6">
        ...
        {frozenRounds.length > 0 && (
          <div className="relative mb-3">
            ...
            >
              {frozenRounds.map((round, index) => (
                isFinal(round.name) && frozenSemifinalIndex >= 0
                  ? null
                  : <option key={`${round.name}-${index}`} value={index}>{isSemifinal(round.name) && frozenFinalIndex >= 0 ? "Semifinale e finale" : round.name}</option>
              ))}
            </select>
        ...
        <div className="flex flex-col gap-4">
          {frozenRoundsToRender.map((round) => (
            <div key={round.name}>
          ...
          {showFrozenFinalStages && frozenChampion && (
            <div className="mt-2 rounded-2xl border border-[#BBFF5E] bg-[rgba(187,255,94,0.10)] px-5 py-6 text-center">
              <Trophy className="mx-auto mb-2 text-[#BBFF5E]" size={30} />
              <p className="text-[11px] font-extrabold uppercase text-[#BBFF5E]">Vincitrice</p>
              <p className="mt-1 font-display text-3xl text-[#FBF3DE]">
                {frozenChampion.winnerSide === 1 ? frozenChampion.team1Name : frozenChampion.team2Name}
              </p>
            </div>
          )}
        </div>
  ...
  return (
    <div className="mt-6">
      ...
      {sortedRounds.length === 0 ? (
        ...
        <div className="relative mb-3">
          ...
          <select
            ...
            value={showFinalStages ? FINAL_STAGES_VALUE : selectedRound?.id ?? ""}
            onChange={(event) => setSelectedRoundId(event.target.value)}
            className="w-full appearance-none rounded-lg border border-[rgba(251,243,222,0.16)] bg-[#0A0B08] px-3 py-3 pr-10 text-sm font-bold text-[#FBF3DE]"
          >
            {sortedRounds.map((round) => (
              !isAdmin && isFinal(round.name) && finalStageRounds.some((item) => isSemifinal(item.name))
                ? null
                : <option key={round.id} value={!isAdmin && isSemifinal(round.name) && finalStageRounds.some((item) => isFinal(item.name)) ? FINAL_STAGES_VALUE : round.id}>
                    {!isAdmin && isSemifinal(round.name) && finalStageRounds.some((item) => isFinal(item.name)) ? "Semifinale e finale" : round.name}
                  </option>
            ))}
          </select>
          <ChevronDown className="pointer-events-none absolute bottom-3.5 right-3 text-[#BBFF5E]" size={16} />
      ...
      {showFinalStages ? finalStageRounds.map((round) => (
        <RoundDetail
          key={round.id}
          round={round}
          matches={allMatches.filter((match) => match.roundId === round.id)}
          canMoveUp={false}
          canMoveDown={false}
          onMove={() => undefined}
          onRename={() => undefined}
